...
class _PrestasiPageState extends State<PrestasiPage> {
  ...
  late Future<PrestasiOrganisasiResponse> _futurePrestasi;

  @override
  void initState() {
    super.initState();
    _futurePrestasi = getPrestasiByOrganisasiId();
  }

  Future<PrestasiOrganisasiResponse> getPrestasiByOrganisasiId() async {
    final Uri url = Uri.parse(
      '$baseUrl/mobile/prestasi_organisasi/${widget.organisasiId}/',
    );
    ...
    print('URL GET PRESTASI: $url');
    print('STATUS GET PRESTASI: ${response.statusCode}');
    print('BODY GET PRESTASI: ${response.body}');
    ...
    if (response.statusCode == 200) {
      return PrestasiOrganisasiResponse.fromJson(body);
    }
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Expanded(
              child: FutureBuilder<PrestasiOrganisasiResponse>(
                ...
                builder: (context, snapshot) {
                  ...
                  final List<PrestasiOrganisasiItem> prestasiList =
                      snapshot.data?.data ?? [];
                  ...
                  return RefreshIndicator(
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 18),
                        Column(
                          children: prestasiList.map((item) {
                            return Padding(
                              padding: const EdgeInsets.only(bottom: 14),
                              child: PrestasiCard(
                                title: item.prestasi?.isNotEmpty == true
                                    ? item.prestasi!
                                    : 'Prestasi',
                                description:
                                    item.detailPrestasi?.isNotEmpty == true
                                        ? item.detailPrestasi!
  ...
  Widget _buildHeroCard() {
    return Container(
      ...
      child: Stack(
        ...
        children: [
          Image.network(
            "https://images.unsplash.com/photo-1523580494863-6f3031224c94?w=900",
            fit: BoxFit.cover,
            errorBuilder: functionImageError,
          ),
          Container(
  ...
  Widget _buildInfoText() {
    return const Text(
      "Daftar pencapaian dan prestasi yang pernah diraih oleh organisasi.",
      style: TextStyle(
        fontSize: 12,
  ...
  Widget _buildEmptyCard() {
    return Container(
      ...
      child: const Text(
        'Belum ada data prestasi untuk organisasi ini.',
        style: TextStyle(
          color: Colors.grey,
          fontSize: 13,
          height: 1.5,
...
class PrestasiOrganisasiResponse {
  List<PrestasiOrganisasiItem>? data;
  Metadata? metadata;

  PrestasiOrganisasiResponse({
    this.data,
    this.metadata,
  });

  PrestasiOrganisasiResponse.fromJson(Map<String, dynamic> json) {
    if (json['data'] != null) {
      data = <PrestasiOrganisasiItem>[];

      if (json['data'] is List) {
        json['data'].forEach((v) {
          data!.add(PrestasiOrganisasiItem.fromJson(v));
        });
      }
    }
...
class PrestasiOrganisasiItem {
  ...
  PrestasiOrganisasiItem.fromJson(Map<String, dynamic> json) {
    id = _toNullableInt(json['id']);
    prestasi = _toNullableString(json['prestasi']);
    detailPrestasi = _toNullableString(json['detail_prestasi']);
  ...
  Map<String, dynamic> toJson() {
    ...
    result['id'] = id;
    result['prestasi'] = prestasi;
    result['detail_prestasi'] = detailPrestasi;
